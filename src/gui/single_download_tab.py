"""Flujo individual de descarga y procesamiento local."""
import threading
import webbrowser
...
from src.core.notification_sound import play_completion_sound
# -------------------------------------------------
...
class SingleDownloadTab(ctk.CTkFrame):
    ...
    def refresh_theme(self):
        ...
        scroll_surface = self.app.get_theme_color("SCROLL_SURFACE", ["gray86", "gray17"])
        for scrollable_name in ("options_scroll_frame", "recode_main_frame"):
            scrollable = getattr(self, scrollable_name, None)
            if scrollable is not None and scrollable.winfo_exists():
                scrollable.configure(fg_color=scroll_surface)
        ...
        # CTk repinta los widgets de forma diferida. Evitamos update_idletasks()
        # aquí porque, si coincide con la rueda del ratón, puede dejar residuos.
    ...
    def _create_widgets(self):
        ...
        url_frame = ctk.CTkFrame(self, corner_radius=12, border_width=1)
        url_frame.pack(pady=(12, 2), padx=12, fill="x")
        ctk.CTkLabel(
            url_frame,
            text="ENLACE DEL VIDEO",
            font=ctk.CTkFont(size=11, weight="bold"),
        ).pack(anchor="w", padx=14, pady=(10, 3))
        url_controls = ctk.CTkFrame(url_frame, fg_color="transparent")
        url_controls.pack(fill="x", padx=10, pady=(0, 10))
        self.url_entry = ctk.CTkEntry(
            url_controls,
            placeholder_text="Pega un enlace de YouTube, Vimeo, TikTok…",
            height=38,
        )
        ...
        self.original_analyze_text = "Analizar enlace"
        self.original_analyze_fg_color = self.ANALYZE_BTN_COLOR
        self.original_analyze_fg_color = self.analyze_button.cget("fg_color")
        
        info_frame = ctk.CTkFrame(self, corner_radius=12, border_width=1)
        ...
        self.options_scroll_frame = ctk.CTkScrollableFrame(
            left_column_container,
            fg_color=self.app.get_theme_color("SCROLL_SURFACE", ["gray86", "gray17"]),
        )
        options_scroll_frame = self.options_scroll_frame
        ...
        ctk.CTkLabel(options_scroll_frame, text="Recursos IA", font=ctk.CTkFont(weight="bold")).pack(fill="x", padx=10, pady=(5, 2))
        ai_resources_frame = ctk.CTkFrame(options_scroll_frame)
        ai_resources_frame.pack(fill="x", padx=5, pady=(0, 10))
        ai_resources_frame.grid_columnconfigure(0, weight=1)
        ...
        self.rembg_status_label.grid(row=0, column=0, padx=10, pady=(8, 5), sticky="ew")
        ...
        self.open_models_folder_button.grid(row=1, column=0, padx=10, pady=(0, 10), sticky="ew")
        ...
        self.recode_main_frame = ctk.CTkScrollableFrame(
            details_frame,
            fg_color=self.app.get_theme_color("SCROLL_SURFACE", ["gray86", "gray17"]),
        )
        ...
        recode_tooltip_text = "Permite convertir el archivo a un formato diferente.\nÚtil para mejorar la compatibilidad, estandarizar el video\no reducir el tamaño del archivo."
        ...
        fps_tooltip_text = "Fuerza una tasa de fotogramas constante (CFR).\n\nEvita problemas de sincronización que pueden aparecer en videos con FPS variable (VFR)."
    ...
    def _open_release_page(self):
        """Las actualizaciones de Xomacito se gestionan localmente."""
        return None
    ...
    def import_local_file_from_path(self, filepath):
        ...
        Usado por la importación local de archivos.
        """
    ...
    def export_preset_file(self):
        """
        Exporta el preset seleccionado como archivo .xomacito_preset.
        """
        ...
        file_path = filedialog.asksaveasfilename(
            defaultextension=".xomacito_preset",
            filetypes=[("Xomacito Preset", "*.xomacito_preset"), ("JSON", "*.json"), ("All Files", "*.*")],
            initialfile=f"{selected_preset_name}.xomacito_preset"
        )
    ...
    def import_preset_file(self):
        """
        Importa un preset de Xomacito y lo agrega a presets personalizados.
        """
        file_path = filedialog.askopenfilename(
            filetypes=[
                ("Xomacito Preset", "*.xomacito_preset"),
                ("Preset compatible anterior", "*.dowp_preset"),
                ("JSON", "*.json"),
                ("All Files", "*.*"),
            ],
            title="Selecciona un preset de Xomacito para importar"
        )
    ...
    def _create_preset_file_content(self, preset_data, preset_name):
        """
        Crea el contenido de un preset de Xomacito con validación.
    ...
    def _validate_preset_file(self, preset_content):
        """
        Valida la integridad de un preset de Xomacito.
            ...
            try:
                ...
                if is_fragment_mode:
                    try:
                        ...
                        # FIX: Xomacito's custom FFmpeg binary freezes when fetching YouTube HTTPS range requests.
    ...
    def on_process_finished(self, success, message, final_filepath, show_dialog=True):
        ...
        if success:
            ...
            lowered_final_message = final_message.lower()
            if final_filepath and any(token in lowered_final_message for token in ("descarga completada", "recodificación completada")):
            ...
            if show_dialog:
                ...
                elif "403" in lowered_message or "forbidden" in lowered_message:
                    dialog_message = friendly_ytdlp_error(final_message)
    ...
    def _run_analysis_subprocess(self, url):
        ...
        try:
            ...
            other_lines = captured_text.strip().splitlines() + log_lines

            if info is None:
                raise yt_dlp.utils.DownloadError(friendly_ytdlp_error("No se recibió información.", other_lines))
    ...
    def on_analysis_complete(self, info, error_message=None):
        try:
            ...
            if not info or error_message:
                ...
                self._clear_subtitle_menus()
                self.update_progress(0, "Análisis fallido. Revisa el mensaje o configura Cookies.")
                return
            self.progress_bar.set(1)
    ...
    def _send_thumbnail_to_image_tools(self):
        ...
        try:
            ...
            import tempfile
            temp_dir = tempfile.mkdtemp(prefix="xomacito_thumbnail_")
            ...
            self.app.tab_view.set("Estudio de Imagen")
