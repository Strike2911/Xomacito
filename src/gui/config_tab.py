...
class ConfigTab(ctk.CTkFrame):
    def __init__(self, master, app, *args, **kwargs):
        ...
        self.about_label = ctk.CTkLabel(self.bottom_frame, text="Creado por Strike |", font=ctk.CTkFont(size=12, weight="bold"))
        self.about_label.pack(side="left", padx=(15, 5), pady=10)
        
        import webbrowser
        
        self.link_yt = ctk.CTkButton(self.bottom_frame, text="YouTube", width=60, height=20, fg_color=self.DOWNLOAD_BTN, text_color=self.DOWNLOAD_TEXT, hover_color=self.DOWNLOAD_HOVER, command=lambda: webbrowser.open("https://www.youtube.com/@ElStrikew"))
        self.link_yt.pack(side="left", padx=5)

        self.link_donate = ctk.CTkButton(self.bottom_frame, text="Ko-fi ☕", width=80, height=20, fg_color=self.TERTIARY_BTN, text_color=self.TERTIARY_TEXT, hover_color=self.TERTIARY_HOVER, command=lambda: webbrowser.open("https://ko-fi.com/strikepoint"))
    ...
    def _setup_sections(self):
        ...
        # ===== Sección: General =====
        frame_general = ctk.CTkScrollableFrame(self.content_container, fg_color=self.SCROLL_SURFACE)
        self.scrollable_sections.append(frame_general)
        ...
        ctk.CTkLabel(
            downloads_group, 
            text="Elimina emojis y símbolos para crear nombres de archivo más compatibles.", 
        ...
        # ===== Sección: Cookies =====
        frame_cookies = ctk.CTkScrollableFrame(self.content_container, fg_color=self.SCROLL_SURFACE)
        self.scrollable_sections.append(frame_cookies)
        ...
        frame_deps = ctk.CTkScrollableFrame(self.content_container, fg_color=self.SCROLL_SURFACE)
        self.scrollable_sections.append(frame_deps)
        ...
        import webbrowser
        ctk.CTkButton(recovery_frame, text="Página Oficial de Xomacito", fg_color=self.DOWNLOAD_BTN, hover_color=self.DOWNLOAD_HOVER, text_color=self.DOWNLOAD_TEXT, command=lambda: webbrowser.open("https://marckdp.github.io/Xomacito/")).pack(anchor="w", padx=20, pady=(5, 15))
        ...
        # ===== Sección: Modelos =====
        frame_models = ctk.CTkScrollableFrame(self.content_container, fg_color=self.SCROLL_SURFACE)
        self.scrollable_sections.append(frame_models)
    ...
    def _console_export(self):
        ...
        if not text.strip():
            return
        default_name = f"xomacito_log_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    def _on_theme_change(self, display_name):
        ...
        if not internal_name or internal_name == getattr(self.app, 'selected_theme_accent', 'midnight_ocean'):
            return
            
        self.app.selected_theme_accent = internal_name
        self.app.theme_selection_explicit = True
        self.app.save_settings()
        ...
        if messagebox.askyesno("Tema Actualizado", 
                               ...
                               "¿Deseas reiniciar Xomacito ahora para aplicar el cambio de acento de forma completa en toda la interfaz (marcos, barras, etc)?"):
    ...
    def _refresh_theme_list(self):
                    ...
                    if file.endswith(".json"):
                        ...
                        try:
                            with open(full_path, 'r', encoding='utf-8') as f:
                                ...
                                display = data.get("ThemeName") or data.get("_INSTRUCCIONES_XOMACITO", {}).get("ThemeName")
                        except:
                            pass
        ...
        current_internal = getattr(self.app, 'selected_theme_accent', 'midnight_ocean')
    ...
    def _install_theme_from_url(self):
        ...
        try:
            ...
            internal_name = theme_data.get("ThemeName") or theme_data.get("_INSTRUCCIONES_XOMACITO", {}).get("ThemeName")
            if internal_name:
    ...
    def _delete_theme(self):
        ...
        if messagebox.askyesno("Confirmar eliminación", f"¿Estás seguro de que quieres eliminar el tema '{display_name}'?\n\nEsta acción no se puede deshacer."):
            try:
                ...
                self.app.selected_theme_accent = "midnight_ocean"
                self.app.save_config()
                
                # Opcional: preguntar si quiere reiniciar ahora
                if messagebox.askyesno("Reiniciar ahora", "¿Quieres reiniciar Xomacito ahora para aplicar los cambios?"):
    ...
    def _populate_rembg_model_rows(self):
        ...
        # Solo este modelo de RMBG 2.0 permite descarga directa desde Xomacito
        ...
        for family_name, models in REMBG_MODEL_FAMILIES.items():
            ...
            if family_name == RMBG2_FAMILY:
                notice = (
                    "Únicamente el modelo 'Automático' puede descargarse directamente desde Xomacito. "
    ...
    def _load_theme_colors(self):
        ...
        self.SCROLL_SURFACE = self.app.get_theme_color("SCROLL_SURFACE", ["gray86", "gray17"])
    ...
    def refresh_theme(self):
        ...
        for scrollable in self.scrollable_sections:
            if scrollable.winfo_exists():
                scrollable.configure(fg_color=self.SCROLL_SURFACE)

        frame_color = self.app.theme_data.get("CTkFrame", {}).get("fg_color", self.SCROLL_SURFACE)
        frame_border = self.app.theme_data.get("CTkFrame", {}).get("border_color", self.CONFIG_CARD_BORDER)
        for structural_frame in (self.sidebar_frame, self.bottom_frame):
            if structural_frame.winfo_exists():
                structural_frame.configure(fg_color=frame_color, border_color=frame_border)
